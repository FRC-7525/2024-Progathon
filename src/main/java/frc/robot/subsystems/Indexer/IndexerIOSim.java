...
public class IndexerIOSim implements IndexerIO {
    ...
    public IndexerIOSim() {
        wheelMotorSim = new DCMotorSim(LinearSystemId.createDCMotorSystem(DCMotor.getKrakenX60(1), 0.000001, 1), DCMotor.getKrakenX60(IndexerConstants.NUM_SPINNER_MOTORS));
		speedController = new PIDController(0, 0, 0);

	@Override
	public void setSetpoint(double setpoint) {
		wheelAppliedVoltage = speedController.calculate(
			wheelMotorSim.getAngularVelocityRPM() / 60.0,
			setpoint
		);

...
public class ElevatorIOReal implements ElevatorIO {
	...
	public ElevatorIOReal() {
		...
		leftConfigurations.CurrentLimits.StatorCurrentLimit =
			LEFT_STRATOR_CURRENT_LIMIT.magnitude();
		...
		rightConfigurations.CurrentLimits.StatorCurrentLimit =
			RIGHT_STRATOR_CURRENT_LIMIT.magnitude();
		...
		pidController.setTolerance(
			ElevatorConstants.POSITION_TOLERANCE.magnitude(),
			ElevatorConstants.VELOCITY_TOLERANCE.magnitude()
		);
	...
	public void runElevator() {
		leftMotorVoltage =
			...
			ffcontroller
				.calculate(MetersPerSecond.of(pidController.getSetpoint().velocity))
				.magnitude();
		rightMotorVoltage =
			pidController.calculate(
	...
	public void zero() {
		...
		if (
			rightMotor.getStatorCurrent().getValueAsDouble() >
				ElevatorConstants.ZEROING_CURRENT_LIMIT.magnitude() ||
			!limitSwitch.get()
		) {
			rightZeroingSpeed = 0;
